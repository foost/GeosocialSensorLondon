# coding: utf-8
# Author: C.J. Hutto MODIFIED by F. Ostermann
...
class SentimentIntensityAnalyzer(object):
    ...
    def polarity_scores(self, text):
        ...
        for chr in text:
            #print(chr)
            #print(chr, ord(chr), prev_space, text_no_emoji)
            if chr in self.emojis:
                ...
                text_no_emoji += chr
                prev_space = chr == ' '
        #print(text_no_emoji, ord(text_no_emoji[-1]))
        text = text_no_emoji.strip()
        #print(text, ord(text[-1]))
    ...
    def sentiment_valence(self, valence, sentitext, item, i, sentiments):
        ...
        #print(item_lowercase)
        #if item_lowercase == 'heart':
            #print(self.lexicon['heart'])
...
if __name__ == '__main__':
    ...
    for t in tw_texts_en:
        vs = analyzer.polarity_scores(t[1])
        if vs['compound'] <= -0.05:
            sentiment = 0
        ...
        london_full_combo_en.append([t[1], vs['compound'], sentiment])
        data = (vs['compound'],t[0])
        cur.execute("""update london_twitter_v3_4326 set sent_vader=(%s) where tweet_id=(%s);""", data)
        conn.commit()
        print(sentiment_count_en)
    #print(london_full_combo_en[0])
    #print(sentiment_count_en)
    ...
    # with open(
    #         'C:/Users/ostermannfo/surfdrive/research/sandbox/LondonPlaceSemantics/VADERmod_london_v3_en_nobots_sample.csv',
    #         'w', encoding='utf-8', newline='') as myfile:
