...
LOG_FILE = "FlickrBuildTermVectors.log"
...
def main():
    ...
    try:
        cur.execute("SELECT gid FROM flickr_table WHERE terms_found IS NULL")
        gids=cur.fetchall()            
    ...
    for gid in gids:
        ...
        try:
            cur2.execute("SELECT title, description, tags_user FROM flickr_table WHERE gid = '"
                        + str_gid + "'")
        ...
        for input_element in input_text[0]:
            unigrams = re.split(' |~', str(input_element))
        ...
        try:
            cur2.execute("""UPDATE flickr_table
